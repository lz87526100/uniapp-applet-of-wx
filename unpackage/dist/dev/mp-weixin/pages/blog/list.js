...
if (!Array) {
  ...
  (_easycom_home_head2 + _easycom_uni_load_more2 + _easycom_uni_dateformat2 + _easycom_uni_icons2)();
}
...
if (!Math) {
  (_easycom_home_head + _easycom_uni_load_more + _easycom_uni_dateformat + _easycom_uni_icons)();
}
const size = 8;
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onMounted(() => {
      common_vendor.index.__f__("log", "at pages/blog/list.vue:149", "博客列表页面加载");
      setupUserInfoListeners();
      getData(true);
    });
    common_vendor.onReachBottom(() => {
      common_vendor.index.__f__("log", "at pages/blog/list.vue:156", "页面触底，触发加载更多");
      loadMore();
    });
    ...
    common_vendor.index.$on("editEvent", () => {
      common_vendor.index.__f__("log", "at pages/blog/list.vue:173", "收到编辑事件，刷新数据");
      page = 1;
    ...
    const setupUserInfoListeners = () => {
      common_vendor.index.$on("userInfoUpdated", (data) => {
        common_vendor.index.__f__("log", "at pages/blog/list.vue:182", "博客列表收到用户信息更新:", data);
    ...
    const updateUserAvatarInList = (userData) => {
      if (!userData.userId || !userData.avatar)
        return;
      common_vendor.index.__f__("log", "at pages/blog/list.vue:191", "开始更新博客列表中的用户头像，用户ID:", userData.userId);
    ...
    const convertCloudFileUrl = async (fileUrl, userId) => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/blog/list.vue:283", "转换云存储URL失败:", error);
      }
    };
    ...
    const handleAvatarError = (event2) => {
      common_vendor.index.__f__("log", "at pages/blog/list.vue:304", "头像加载失败:", event2);
    };
    async function getData(reset = true) {
      ...
      try {
        common_vendor.index.__f__("log", "at pages/blog/list.vue:317", "获取博客列表数据，页码:", page);
        const { errCode, data } = await articlesCloudObj.list({ page, size });
        if (errCode === 0) {
          common_vendor.index.__f__("log", "at pages/blog/list.vue:321", "获取到数据条数:", data ? data.length : 0);
          if (reset) {
          ...
          common_vendor.index.__f__("log", "at pages/blog/list.vue:332", "数据加载完成，列表长度:", articlesList.value.length, "是否完成:", finished.value);
          if (articlesList.value.length > 0) {
            await checkFavoritesStatus();
          }
        } else {
          common_vendor.index.__f__("error", "at pages/blog/list.vue:340", "获取数据失败:", errCode);
        ...
        common_vendor.index.__f__("error", "at pages/blog/list.vue:347", "获取数据异常:", error);
        common_vendor.index.showToast({
          title: "网络错误",
          icon: "none"
        });
    ...
    async function toggleFavorite(articleId, item) {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/blog/list.vue:372", "切换收藏状态:", { articleId, currentStatus: item.isFavorited });
        if (item.isFavorited) {
        ...
        common_vendor.index.__f__("error", "at pages/blog/list.vue:390", "切换收藏状态失败:", error);
    ...
    async function checkFavoritesStatus() {
      try {
        ...
        if (articleIds.length === 0) {
          common_vendor.index.__f__("log", "at pages/blog/list.vue:402", "没有文章需要检查收藏状态");
          return;
        }
        common_vendor.index.__f__("log", "at pages/blog/list.vue:406", "🔄 开始检查收藏状态，文章数量:", articleIds.length);
        const favoritesStatus = await common_style_favorites.favoritesManager.batchCheckFavorites(articleIds);
        common_vendor.index.__f__("log", "at pages/blog/list.vue:409", "✅ 收藏状态检查完成:", favoritesStatus);
        ...
        common_vendor.index.__f__("error", "at pages/blog/list.vue:418", "检查收藏状态失败，设置所有为未收藏:", error);
        articlesList.value.forEach((item) => {
          if (item) {
            item.isFavorited = false;
          }
    ...
    function onRefresh() {
      ...
      common_vendor.index.__f__("log", "at pages/blog/list.vue:433", "下拉刷新");
      getData(true);
    }
    function loadMore() {
      if (loadingMore.value || finished.value || loading.value) {
        common_vendor.index.__f__("log", "at pages/blog/list.vue:441", "跳过加载: loadingMore=", loadingMore.value, "finished=", finished.value, "loading=", loading.value);
        return;
      }
      common_vendor.index.__f__("log", "at pages/blog/list.vue:445", "触底加载更多，页码:", page + 1);
      loadingMore.value = true;
      page++;
      getData(false);
    }
    ...
    common_vendor.onUnmounted(() => {
      common_vendor.index.__f__("log", "at pages/blog/list.vue:493", "博客列表页面卸载");
    ...
    async function debugLoginStatus() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/blog/list.vue:515", "🔍 登录状态调试结果:", result);
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/blog/list.vue:517", "调试失败:", error);
      }
    }
    common_vendor.onMounted(() => {
      debugLoginStatus();
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        e: common_vendor.f(articlesList.value, (item, k0, i0) => {
          var _a, _b;
          return common_vendor.e({
            a: getUserAvatar(item.user_id[0]),
            b: common_vendor.o(handleAvatarError, item._id),
            c: common_vendor.o(($event) => onAvatarLoad(item.user_id[0]), item._id),
            d: common_vendor.t(item.user_id[0].nickname || "匿名"),
            e: "f909dea6-2-" + i0,
            f: common_vendor.p({
            ...
            h: common_vendor.o(($event) => goDetail(item._id), item._id),
            i: (_a = item.pics) == null ? void 0 : _a.length
          }, ((_b = item.pics) == null ? void 0 : _b.length) ? {
            j: common_vendor.f(item.pics, (pic, idx, i1) => {
              return {
                a: idx,
                b: pic.url,
                c: common_vendor.o(($event) => preview(item.pics, idx), idx)
              };
            })
          } : {}, {
            k: common_vendor.t(item.view_count || 0),
            l: "f909dea6-3-" + i0,
            m: common_vendor.p({
            ...
            n: common_vendor.o(($event) => toggleFavorite(item._id, item), item._id),
            o: common_vendor.unref(utils_common.isPermission)(item.user_id[0]._id)
            ...
            r: common_vendor.o(($event) => remove(item._id), item._id)
          } : {}, {
            s: item._id,
            t: common_vendor.o(touchStart, item._id),
            v: common_vendor.o(($event) => touchEnd(item._id), item._id)
          });
        }),
        f: loadingMore.value
      }, loadingMore.value ? {
